...
def make_background(theme_dir, output_dir, screen_w=320, screen_h=480):
    ...
    # Fallback images dir — for themes that don't have their own icons (e.g. loki_dark)
    fallback_images_dir = os.path.join(os.path.dirname(theme_dir), 'loki', 'images')
    ...
    # Title baked into background — use theme's own title font
    display_name = theme.get('display_name', 'LOKI')
    xp_icon_size = int(h['h'] * 0.7)
    xp_icon_x = screen_w // 2 - xp_icon_size - 4
    max_title_w = xp_icon_x - 10

    # Find the theme's title font
    title_font_path = None
    fonts_dir = os.path.join(theme_dir, 'fonts')
    if os.path.exists(fonts_dir):
        for fname in ['title.ttf', 'title.TTF']:
    ...
    try:
        # Scale up title_font_size for CYD (320px wide vs pager's 222px)
        orig_size = int(theme.get('title_font_size', 20))
        font_size = int(orig_size * screen_w / 222.0)
        font_size = min(font_size, int(h['h'] * 0.85))  # Don't exceed header height
        if title_font_path:
            title_font = ImageFont.truetype(title_font_path, font_size)
        else:
            title_font = ImageFont.truetype("/System/Library/Fonts/Menlo.ttc", font_size)

        # Auto-shrink if too wide for header
        while draw.textlength(display_name, font=title_font) > max_title_w and font_size > 10:
            font_size -= 1
            if title_font_path:
                title_font = ImageFont.truetype(title_font_path, font_size)
            else:
    ...
    # Vertically center title in header area, accounting for font metrics
    bbox = title_font.getbbox(display_name)
    font_height = bbox[3] - bbox[1]
    top_offset = bbox[1]  # How far above baseline the font renders
    title_y = (h['h'] - font_height) // 2 - top_offset
    draw.text((5, title_y), display_name, fill=title_color, font=title_font)

    # XP icon (gold rune) baked in (value drawn dynamically)
    xp_icon_size = int(h['h'] * 0.7)
    xp_icon_path = None
    for ext in ['.png', '.bmp']:
        p = os.path.join(images_dir, f'gold{ext}')
        if os.path.exists(p): xp_icon_path = p; break
    if not xp_icon_path and os.path.exists(fallback_images_dir):
        for ext in ['.png', '.bmp']:
            p = os.path.join(fallback_images_dir, f'gold{ext}')
            if os.path.exists(p): xp_icon_path = p; break
    ...
    # Kill feed area — always black for readability with colored text
    kf = layout['killfeed']
    draw.rectangle([kf['x'], kf['y'], kf['x'] + kf['w'], kf['y'] + kf['h']], fill=(0, 0, 0))
